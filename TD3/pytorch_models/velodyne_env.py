...
def check_pos(x, y):
    ...
    if 3.35 > x > 1.35 and 4.37 > y > 2.37:
        goal_ok = False

    if -1.27 > x > -3.77 and -1.62 > y > -3.12:
        goal_ok = False

    if -2.9 > x > -4.4 and 3.37 > y > 1.37:
        goal_ok = False

    if 2.05 > x > 0.55 and -0.35 > y > -1.85:
        goal_ok = False
...
class GazeboEnv:
    ...
    def __init__(self, launchfile, environment_dim):
        ...
        # subprocess.Popen(["roslaunch", "-p", port, "/home/ubuntu/Code/DRL-robot-navigation/catkin_ws/src/multi_robot_scenario/launch/empty_world.launch"])
        # time.sleep(5)
        # 启动 Gazebo 仿真
    ...
    def odom_callback(self, od_data):
        ...
        self.vx = od_data.twist.twist.linear.x
        self.wz = od_data.twist.twist.angular.z
    ...
    def step(self, action):
        ...
        robot_state = [distance, theta, self.vx, self.wz, action[0], action[1]]
        state = np.append(laser_state, robot_state)  # 组合 LiDAR 数据与机器人状态

        # 计算奖励
        reward = self.get_reward(target, collision, self.vx, self.wz, theta, min_laser, distance)
    ...
    def random_box(self):
        ...
        for i in range(4):
            ...
            box_state.reference_frame = "world"  # 使用世界坐标系
    ...
    # 计算奖励
    @staticmethod
    def get_reward(target, collision, vx, wz, angle, min_laser, distance):
        if target:
            return 1600.0
        elif collision:
            return -200.0
